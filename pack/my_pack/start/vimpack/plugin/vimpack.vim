...
if !exists("g:vimpack_path")
    if has("unix")
        vimpack_path = "~/.vim/pack/vimpack/start/"
    else
        vimpack_path = substitute(system('echo %USERPROFILE%'), '[\n ]', '', 'g') .. '\vimfiles\pack\vimpack\start\'
    endif
else
...
command! VimPackInstall :call InstallAll()
command! VimPackUpdate :call UpdateAll()
command! VimPackClean :call UpdateAll()
